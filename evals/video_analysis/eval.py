...
from evals.video_analysis.utils import (
    make_transforms,
    ClipAggregation,
    FrameAggregation
)
...
def main(args_eval, resume_preempt=False):
    ...
    metadata_json_path = args_data.get('json_config_path')
    real_video_paths = args_data.get('real_video_paths')
    fake_video_path = args_data.get('fake_video_path')
    ...
    folder = os.path.join(pretrain_folder, 'video_analysis/')
    ...
    data_loader= make_dataloader(
        ...
        rank=rank,
        training=True, # This argument is not used in InternVidDataset
        )
    ipe = len(data_loader)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        logger.info('[%5d] video_analysis: %.3f%%' % (epoch + 1, train_acc))
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.amp.autocast(device_type=device.type, dtype=torch.float16, enabled=use_bfloat16):
            # Real and fake video clips
            clips_tuple, labels, clip_indices = data
            real_clips, fake_clips = clips_tuple[0], clips_tuple[1]
            real_labels, fake_labels = labels[0], labels[1]
            
            # Forward prediction for real and fake video clips
            for clips, labels in [(real_clips, real_labels), (fake_clips, fake_labels)]:  
                # Load data and put on GPU
                clips = [
                    [dij.to(device, non_blocking=True) for dij in di]  # iterate over spatial views of clip
                    for di in clips  # iterate over temporal index of clip
                ]
...
def make_dataloader(
    ...
    data_loader, _ = init_data_internvid(
        data=dataset_type,
        real_video_paths = real_video_paths,
        fake_video_path = fake_video_path,
